...
\begin{definition}
    ...
    The rank of a polynomial $p$ is the pair $(mainVariable(p), deg(p))$ ordered lexicographically.
\end{definition}
...
\begin{definition}
    ...
    Pseudo-division of $g$ by $f$ with respect to $mainVariable(f)$.
...
\begin{definition}
    ...
    Difference from Standard: The filter condition includes $mainVariable(p) > mainVariable(B)$.
\end{definition}
...
\begin{lemma}
    ...
    $q$ is reduced w.r.t. $p$ if $mainVariable(q) < mainVariable(p)$.
\end{lemma}
...
\begin{lemma}
    ...
    If $mainVariable(p) = mainVariable(q)$, then $q$ is reduced with respect to $p$ if and only if $q < p$.
\end{lemma}
...
\begin{lemma}
    ...
    \leanok
    \uses{def:mainVariable, def:reducedTo}
    $mainVariable(p) < mainVariable(q)$ if $p ≤ q$ and $q$ is reduced with respect to $p$.
\end{lemma}
...
\begin{lemma}
    ...
    $mainVariable(init(p)) < mainVariable(p)$ for non-constant $p$.
\end{lemma}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:pseudo}
    The remainder of $g$ by $f$ is $g$ if $deg_c(g) = 0$ where $c = mainVariable(f)$.
\end{theorem}
...
\begin{theorem}
    ...
    If $p$ is in $S$ and $mainVariable(p) \neq \bot$, then $init(p)$ is in $S$.
\end{theorem}
