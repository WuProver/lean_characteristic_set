...
\begin{definition}
    ...
    \leanok
    \uses{def:initialOf, def:initial, def:class}
...
\begin{definition}
    ...
    Difference from Standard: The filter condition includes $cls(p) > cls(B)$.
\end{definition}
...
\begin{definition}
    ...
    \leanok
    \uses{def:TriangulatedSet, def:HasBasicSet, def:setPseudo, inst:TriangulatedSet.wellFoundedLT, thm:basicSet_append_lt_of_exists_reducedToSet}
...
\begin{definition}
    ...
    \leanok
    \uses{def:characteristicSet, thm:initialOf_reducedToSet_of_cls_ne_bot, thm:characteristicSet_isAscendingSet, thm:characteristicSet_le_basicSet}
...
\begin{lemma}
    \label{lem:degreeOf_add_eq_of_degreeOf_lt}
    \lean{MvPolynomial.degreeOf_add_eq_of_degreeOf_lt}
    \leanok
    $deg_i(p + q) = deg_i(p)$ if $deg_i(q) < deg_i(p)$.
\end{lemma}
...
\begin{lemma}
    \label{lem:reducedTo_iff_gt_of_cls_eq}
    \lean{MvPolynomial.reducedTo_iff_gt_of_cls_eq}
    \leanok
    \uses{def:class, def:reducedTo, def:MvPolynomial.rank}
    If $cls(p) = cls(q)$, then $q$ is reduced with respect to $p$ if and only if $q < p$.
\end{lemma}

\begin{lemma}
    \label{lem:cls_lt_of_reducedTo_of_le}
    \lean{MvPolynomial.cls_lt_of_reducedTo_of_le}
    \leanok
    \uses{lem:reducedTo_iff_gt_of_cls_eq}
    $cls(p) < cls(q)$ if $p ≤ q$ and $q$ is reduced with respect to $p$.
\end{lemma}
...
\begin{lemma}
    \label{lem:initialOf_add_eq_of_degreeOf_lt}
    \lean{MvPolynomial:initialOf_add_of_degreeOf_lt}
    \leanok
    \uses{def:initialOf, lem:degreeOf_add_eq_of_degreeOf_lt}
    $init_i(p + q) = init_i(p)$ if $deg_i(q) < deg_i(p)$.
\end{lemma}
...
\begin{theorem}
    \label{thm:degreeOf_mul}
    \lean{MvPolynomial.initialOf_mul}
    \leanok
    \uses{lem:initialOf_add_eq_of_degreeOf_lt, lem:initialOf_eq_of_degreeOf_eq_zero, thm:initialOf_decomposition}
    $deg_i(p \cdot q) = deg_i(p) + deg_i(q)$ if there is no zero divisors in the coefficient ring.
\end{theorem}

\begin{theorem}
    \label{thm:initialOf_mul}
    \lean{MvPolynomial.initialOf_mul}
    \leanok
    \uses{lem:initialOf_add_eq_of_degreeOf_lt, lem:initialOf_eq_of_degreeOf_eq_zero, thm:initialOf_decomposition}
...
\begin{theorem}
    ...
    \leanok
    \uses{inst:MvPolynomial.wellFoundedLT, def:TriangulatedSet, def:TriangulatedSet.rank}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:takeConcat, def:reducedToSet, def:TriangulatedSet.rank, lem:reducedTo_iff_gt_of_cls_eq}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:isRemainder, def:pseudo, thm:degreeOf_mul, thm:initialOf_mul}
...
\begin{lemma}
    ...
    \leanok
    \uses{def:pseudo, lem:degreeOf_initialOf_le}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:isSetRemainder, def:setPseudo, thm:pseudo_remainder_isRemainder, lem:degreeOf_pseudo_remainder_le_of_degreeOf_eq_zero}
    A remainder $r$ of $g$ by a set $S$ is reduced with respect to $S$ and
    satisfies $(\prod S_i^{e_i}) \cdot g = \sum q_i \cdot S_i + r$ for some $\{e_i\}$ and $\{q_i\}$.
\end{theorem}

\begin{theorem}
    \label{thm:initialOf_reducedToSet_of_cls_ne_bot}
    \lean{AscendingSet.initialOf_reducedToSet_of_cls_ne_bot}
    \leanok
    \uses{def:initial, def:reducedToSet, def:class, def:isAscendingSet}
    If $p$ is in $S$ and $cls(p) \neq \bot$, then $init(p)$ is in $S$.
\end{theorem}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:isStandardAscendingSet, def:isMinimal, def:HasBasicSet, def:StandardAscendingSet.basicSet, lem:cls_lt_of_reducedTo_of_le, thm:takeConcat_lt_of_reducedToSet}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:isWeakAscendingSet, def:isMinimal, def:HasBasicSet, def:WeakAscendingSet.basicSet, thm:takeConcat_lt_of_reducedToSet}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:HasBasicSet, thm:takeConcat_lt_of_reducedToSet}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:TriangulatedSet, def:isCharacteristicSet, def:initialProd, thm:vanishingSet_diff_initialProd_subset}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:TriangulatedSet, def:isCharacteristicSet, def:initial, thm:vanishingSet_diff_initialProd_eq}
...
\begin{theorem}
    ...
    \leanok
    \uses{def:characteristicSet, def:isCharacteristicSet, thm:setPseudo_remainder_isSetRemainder}
...
\begin{theorem}
    \label{thm:remainder_eq_zero_of_mem_zeroDecomposition}
    \lean{MvPolynomial.List.remainder_eq_zero_of_mem_zeroDecomposition}
    \leanok
    \uses{def:zeroDecomposition, thm:characteristicSet_isCharacteristicSet}
    $\forall CS \in \mathcal{ZD}(l), g \in l$, $0$ is the remainder of $g$ by $CS$.
\end{theorem}
...
\begin{theorem}
    ...
    $$ Zero(PS) = \bigcup_{CS \in \mathcal{ZD}(l)} Zero(CS/IP(CS)) $$.
\end{theorem}